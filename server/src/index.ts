...
console.error('Error creating room:', error, 'for player:', playerName);
  ...
  socket.on('join_room', (roomCode, playerName, callback) => {
    try {
      console.log(`🔍 Join room request: code="${roomCode}", player="${playerName}"`);
      
      // Debug: Show current rooms
      const roomStats = roomManager.getRoomStats();
      console.log(`📊 Current server state: ${roomStats.totalRooms} rooms, ${roomStats.totalPlayers} players`);
